...
namespace Engage.Dnn.Publish.Services
{
    ...
    public class PublishServices : WebService
    {
        ...
        public Pair[] GetArticlesByCategory(int categoryId)
        {
            ...
            if (category != null)
            {
                foreach (DataRow articleRow in Item.GetAllChildren(ItemType.Article.GetId(), category.ItemId, Util.RelationshipType.ItemToParentCategory.GetId(), Util.RelationshipType.ItemToRelatedCategory.GetId(), category.PortalId).Tables[0].Rows)//Article.GetArticles(category.ItemId, category.PortalId).Rows)
                {
